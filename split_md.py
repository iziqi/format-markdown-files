# 将需要分割的md文件放在original_notes文件夹中，然后运行split_md.py
# 会先按照一级标题创建文件夹，每个二级标题及其内容会以单独md文件保存在对应的一级标题文件夹中
# 分割后的md文件hv 保存在separate_notes文件夹中
...
if __name__ == '__main__':
    ...
    # 初始化notes文件夹
    old_notes_folder = os.path.join(os.getcwd(), 'original_notes')
    new_notes_folder = os.path.join(os.getcwd(), 'separate_notes')

    #初始化separate_notes文件夹
    shutil.rmtree(new_notes_folder, ignore_errors=True) # 清空目标文件夹，包括文件夹本身
    if not os.path.exists(new_notes_folder): 
    ...
    for note in os.listdir(old_notes_folder):
        ...
        note = os.path.join(old_notes_folder, note)

        # 创建存放分割后notes的文件夹
        note_folder = os.path.join(new_notes_folder, note_name)

        # 分割该note
        split_md(note, note_folder)